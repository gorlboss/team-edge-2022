...
currentNumber = 1
while 0 < currentNumber < 100:
  if test_prime(currentNumber) == True:
    print(currentNumber)
  currentNumber += 1
...
#correctAns1, correctAns2 = 63, 15
# #2. Make user guess numbers
# isWrong = True
# while isWrong:
#   ans = input("Enter two numbers (00,00) >> ")
#   ansList = ans.split(",") #['00', '00']
#   userAns1 = int(ansList[0]) # Now it's an interger
#   userAns2 = int(ansList[1])
# #3. If user is correct tell them such, if they are wrong make them guess again
#   if (userAns1 == correctAns1) and (userAns2 == correctAns2):
...
def response(response):
  if response == "baxter":
    print("*Whisper* You've guessed the secret word. You are free. Enjoy.")
    return False
  else:
...
while keep_asking:
    keep_asking = response(prompt_user())
